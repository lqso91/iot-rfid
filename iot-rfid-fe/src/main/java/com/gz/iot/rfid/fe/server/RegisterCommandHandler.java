...
import com.gz.iot.rfid.core.packet.RegisterAckSegment;
import com.gz.iot.rfid.core.packet.RegisterSegment;
...
public class RegisterCommandHandler extends SimpleChannelInboundHandler<ByteBuf> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, ByteBuf byteBuf) throws Exception {
        log.info("RegisterCommandHandler channelRead0 readableBytes: {}", byteBuf.readableBytes());
        // 解析数据包报文头
        HeaderSegment header = new HeaderSegment(byteBuf);
        // 校验指令码，如果是注册指令则处理，否则丢弃
        if (Command.REGISTER == header.getCommand()) {
            RegisterSegment registerSegment = new RegisterSegment(byteBuf);
            log.info(registerSegment.toString());
            ...
            headerAck.setDeviceID(header.getDeviceID());
